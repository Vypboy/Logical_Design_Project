...
void scan_key_matrix_update(){
	...
	for(i=0;i<MAX_ROW;i++)     
	{
		...
		{ 
			if((PORT_BUTTON & arrayMaskInputOfKey[j]) == 0){
                if(i==0&&j<3) key_code[i*MAX_ROW+j+1] = key_code[i*MAX_ROW+j+1] + 1;
                else if(i==2&&j<3) key_code[i*MAX_ROW+j-1] = key_code[i*MAX_ROW+j-1] + 1;
                else if(j==3) key_code[10+i] = key_code[10+i] + 1;
                else if(i==3&&j==0) key_code[14] = key_code[14] + 1;
                else if(i==3&&j==1) key_code[0] = key_code[0] + 1;
                else if(i==3&&j==2) key_code[15] = key_code[15] + 1;
                else key_code[i*MAX_ROW+j] = key_code[i*MAX_ROW+j] + 1;
            }  
				
			else
            {
                if(i==0&&j<3) key_code[i*MAX_ROW+j+1] = 0;
                else if(i==2&&j<3) key_code[i*MAX_ROW+j-1] = 0;
                else if(j==3) key_code[10+i] = 0;
                else if(i==3&&j==0) key_code[14] = 0;
                else if(i==3&&j==1) key_code[0] = 0;
                else if(i==3&&j==2) key_code[15] = 0;
                else key_code[i*MAX_ROW+j] = 0;
            }   
		}
	}
}
